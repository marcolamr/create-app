...
function filterInput(user: any, feature: string, input: any, target: any = undefined) {
  validateUser(user);
  validateFeature(feature);
  validateInput(input);
...
function filterOutput(user: any, feature: string, output: any) {
  validateUser(user);
  validateFeature(feature);
  validateOutput(output);

  let filteredOutputValues: any = {};
  ...
  if (feature === 'read:user') {
    filteredOutputValues = {
      ...
      created_at: output.created_at ?? output.createdAt,
      updated_at: output.updated_at ?? output.updatedAt,
    };
  }
  ...
  if (feature === 'read:user:list') {
    filteredOutputValues = output.map((user: any) => ({
      id: user.id,
  ...
  if (feature === 'read:firewall' && can(user, feature)) {
    ...
    filteredOutputValues.events.forEach((event: any) => delete event.originator_ip);
  }
...
function canRequest(request: NextRequest, feature: string) {
